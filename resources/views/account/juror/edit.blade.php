<x-app-layout>
  <x-drawer.drawer-superadmin>
    @section('pageTitle', 'Edit Juror Request Form')

    @section('content')
      <form method="POST" action="{{ route('juror.update', ['juror'=>$juror]) }}">
        @method('PUT')
        @csrf
        ...
        <x-form-project-layout>
          ...
          <x-slot:formElement>
            <div class="grid grid-cols-3 gap-6">
              <div class="col-span-3">
                ...
                @if ($juror->status == "pending")
                  <select id="status-list" class="form-select appearance-none block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" name="status" :value="old('status')" required autofocus>
                ...
                @if ($juror->status == "rejected")
                  <select id="status-list" class="form-select appearance-none block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" name="status" :value="old('status')" required autofocus>
                    <option disabled selected>Rejected</option>
                    <option value="approved">Approved</option>
                  </select>      
                @endif 
              </div>
            ...
            <div class="grid grid-cols-3 gap-6" id="displayReasonField" style="display: none">
              <div class="col-span-3">
                <label for="juror-reason" class="block text-md font-normal text-gray-700 mb-3">Reason</label>
  
                <input id="reason" type="text" name="reason" class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-slate-50 bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-slate-50 focus:border-blue-600 focus:outline-none" :value="old('reason')" autofocus >
              </div>
            </div>

            <div class="grid grid-cols-3 gap-6" id="displayReviewerSection" style="display: none">
              <div class="col-span-3">
                <label for="juror-reviewer" class="block text-md font-normal text-gray-700 mb-3">Reviewer</label>
  
                <input id="reviewer" type="text" name="reviewer" class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-gray-200 bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-slate-50 focus:border-blue-600 focus:outline-none" value="{{ $currentUser->title_user }} {{ $currentUser->fullname }}" required autofocus readonly >
              </div>
            </div>
          </x-slot:FormElement>
        </x-form-project-layout>
